...
export default function ServiceModal({ isOpen, onClose, service, onSave }: ServiceModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.error(t('notifications.saveError', { error: technicalMsg }));

            if (technicalMsg.includes('column') || technicalMsg.includes('name_he')) {
                toast.error(t('notifications.migrationError'), { duration: 10000 });
            }
        } finally {
            setIsLoading(false);
        }
    ...
    const handleAutoTranslate = async (targetLang: 'pt' | 'he') => {
        if (!formData.name_en && !formData.description_en) {
            toast.error(t('notifications.fillEnglishFirst'));
            return;
        }

        setIsTranslating(true);
        const langName = targetLang === 'pt' ? 'Portuguese' : 'Hebrew';
        const loadingToast = toast.loading(t('notifications.translating', { lang: langName }));
        ...
        try {
            ...
            toast.success(t('notifications.translateSuccess'), { id: loadingToast });
        } catch (error) {
            console.error('Translation failed:', error);
            toast.error(t('notifications.translateError'), { id: loadingToast });
        } finally {
                ...
                <div className="p-6">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-xl font-bold text-[#171717] dark:text-admin-dark-text-primary">
                            {service ? t('serviceModal.editTitle') : t('serviceModal.addTitle')}
                        </h2>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {/* Image Upload */}
                        <div>
                            <label className="block text-xs font-bold text-[#a3a3a3] uppercase tracking-wider mb-2">{t('serviceModal.imageLabel')}</label>
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    <label className="cursor-pointer flex items-center justify-center gap-2 w-full border-2 border-dashed border-[#f5f5f5] dark:border-admin-dark-border rounded-xl p-4 hover:bg-[#fafafa] dark:hover:bg-admin-dark-bg transition-all group">
                                        ...
                                        <span className="text-sm font-medium text-[#171717] dark:text-admin-dark-text-primary transition-colors">{t('serviceModal.uploadImage')}</span>
                                        <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
                                    </label>
                                    <p className="text-[10px] text-[#a3a3a3] mt-1 text-center font-medium">{t('serviceModal.imageHint')}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between pb-2 border-b border-[#f5f5f5] dark:border-admin-dark-border mb-4 transition-colors">
                            <h3 className="text-sm font-bold text-[#171717] dark:text-admin-dark-text-primary">{t('serviceModal.detailsTitle')}</h3>
                        ...
                        <div className="space-y-4">
                            {activeLang === 'en' ? (
                                <>
                                    <div>
                                        <label className="block text-xs font-bold text-[#a3a3a3] uppercase tracking-wider mb-1">{t('serviceModal.nameLabel', { lang: 'English' })}</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-[#a3a3a3] uppercase tracking-wider mb-1">{t('serviceModal.descriptionLabel', { lang: 'English' })}</label>
                                        <textarea
                                            ...
                                            placeholder={t('serviceModal.descriptionPlaceholder', { lang: 'English' })}
                                        />
                                    </div>
                                </>
                            ) : activeLang === 'pt' ? (
                                <>
                                    <div className="flex justify-between items-center mb-1">
                                        <label className="block text-xs font-bold text-[#a3a3a3] uppercase tracking-wider">{t('serviceModal.nameLabel', { lang: 'Portuguese' })}</label>
                                        <button
                                            type="button"
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-[#a3a3a3] uppercase tracking-wider mb-1">{t('serviceModal.descriptionLabel', { lang: 'Portuguese' })}</label>
                                        <textarea
                                            value={formData.description_pt || ''}
                                            onChange={e => setFormData({ ...formData, description_pt: e.target.value })}
                                            className="w-full border border-[#f5f5f5] dark:border-admin-dark-border dark:bg-admin-dark-bg rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-[#171717] dark:focus:ring-white outline-none h-24 resize-none dark:text-admin-dark-text-primary transition-all"
                                            placeholder={t('serviceModal.descriptionPlaceholder', { lang: 'Portuguese' })}
                                        />
                                    </div>
                                </>
                            ) : (
                                <>
                                    <div className="flex justify-between items-center mb-1">
                                        <label className="block text-xs font-bold text-[#a3a3a3] uppercase tracking-wider">{t('serviceModal.nameLabel', { lang: 'Hebrew' })}</label>
                                        <button
                                            type="button"
                                            disabled={isTranslating}
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-[#a3a3a3] uppercase tracking-wider mb-1">{t('serviceModal.descriptionLabel', { lang: 'Hebrew' })}</label>
                                        <textarea
                                            dir="rtl"
                                            ...
                                            placeholder={t('serviceModal.descriptionPlaceholder', { lang: 'Hebrew' })}
                                        />
                                    </div>
                                </>
                            )}
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-medium text-[#171717] dark:text-admin-dark-text-primary">
                                {formData.is_active ? t('serviceModal.activeLabel') : t('serviceModal.inactiveLabel')}
                            </span>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-[#f5f5f5] dark:border-admin-dark-border transition-colors">
                            ...
                            >
                                ...
                                {service ? t('serviceModal.saveChanges') : t('serviceModal.createService')}
                            </button>
                        </div>
                    </form>
                </div>
